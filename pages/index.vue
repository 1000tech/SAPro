...
<template>
    <header class="hero-section pt-10 px-4 md:px-10 border-2 border-[#3790d8] rounded-b-[15px] flex flex-col lg:flex-row justify-center text-center lg:text-left items-center gap-8 relative overflow-hidden">
        <div class="hero-content relative flex-1 max-w-[600px]">
            <h1 class="text-[#3790d8] mb-4 relative z-2">
                {{ page?.title || 'Home' }}
            </h1>
            <p v-if="page?.hero_subtitle" class="text-lg text-[#2c7bc4] font-semibold mb-6">
                {{ page.hero_subtitle }}
            </p>
            <div class="text-base text-[#555] dark:text-gray-300 mb-8 leading-7">
            ...
            <div class="flex gap-4 flex-wrap flex-col md:flex-row items-center md:items-stretch justify-center lg:justify-start lg:pb-10">
                ...
                >
                    {{ page.hero_buttons.secondary_label }}
                </button>
            </div>
        </div>
    ...
    <section id="services" class="services-overview py-8 px-4 md:py-10 md:px-10">
        <h2 class="text-center text-[#3790d8] mb-8 font-bold">
            {{ page?.services_title }}
        </h2>
        <div class="services-grid grid gap-8 max-w-[1200px] mx-auto">
            <div
                v-for="(s, idx) in (page?.services || [])"
                :key="idx"
                class="service-card p-8 rounded-xl text-center shadow-lg border border-[#3790d8] dark:border-gray-600 dark:bg-gray-800 dark:text-white relative overflow-hidden hover:scale-105 hover:shadow-2xl hover:border-[#feeacf] dark:hover:border-gray-400 transition-all"
            >
                <Icon :name="s.icon ?? ''" class="text-[#3790d8] dark:text-blue-400" />
                <h3 class="text-lg text-[#3790d8] dark:text-blue-400 mb-4 font-semibold">
                    {{ s.title }}
                </h3>
                <p class="text-[#666] dark:text-gray-300 leading-6">
                    {{ s.description }}
                </p>
            </div>
        </div>
    ...
    <section class="why-choose-us py-8 px-4 md:py-10 md:px-10">
        <h2 class="text-center text-2xl lg:text-2.2xl text-[#3790d8] mb-12 font-bold">
            {{ page?.why_title }}
        </h2>
